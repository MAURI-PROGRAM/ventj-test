...
import { SaleStatus } from 'src/app/interfaces/saleStatus';
...
import { ProductEditService } from 'src/app/services/product-edit.service';
...
export class ProductFormComponent implements OnChanges {
  ...
  ngOnChanges() {
    ...
    this.productService.getProduct('3208ca7e-93e5-402d-b03a-224f17c527e6').subscribe((data) => {
        this.productToEdit = data;
        if (this.productToEdit) {
          /* this.setProductToEdit(this.productToEdit);
          this.getCategories();
          this.getTags();
          this.getConditions();
  ...
  async editingTagProduct() {
    ...
    this.productToEdit.tags?.forEach((item) => {
      const tag = this.tags?.find((x) => x.id === item.id);
      if (tag) {
        tag.selected = false;
        this.addTagList(tag);
      }
  ...
  editingColorProduct() {
    ...
    this.productToEdit.color?.forEach((item) => {
      const color = this.colors?.find((x) => x.code === item.code);
      if (color) {
  ...
  editingContactMethods() {
    this.productToEdit.availabledContactMethods.forEach((item) => {
      const contact = this.availabledContactMethods?.find(
        (x) => x.type === item.type
      );
      if (contact) {
  ...
  editingSalestatus() {
    const saleStatus = this.statusSaleStatus?.find(
  ...
  selectColor(value: Color) {
    ...
    const indexControl = this.colorsForm.controls.findIndex(
  ...
  removeContactMethodFromList(contactMethod: availabledContactMethods) {
    const indexControl = this.contactMethodsForm.controls.findIndex(
  ...
  removeCategoryFromList(category: Category) {
    const indexControl = this.categoriesForm.controls.findIndex(
  ...
  removeTagFromList(tag: Tag) {
    const indexControl = this.tagsForm.controls.findIndex(
